<template>
  <header class=" border-b-1 border-black">
    ...
    <div class="top-header-container  lg:border-b-2 py-3 lg:py-7" :class="{ scrolled }">
      <div class="top-header container px-0">
        <div class="px-3 flex items-center   justify-between w-full bg-white mb-2 xl:container  xl:px-0">
          <div class="flex gap-3">
            <Icon icon="material-symbols-light:menu" class="lg:hidden text-3xl cursor-pointer"
              @click="openMobileNav()" />


            <RouterLink to="/location" class="hidden lg:block">
              <div class="hidden  lg:flex our-location  items-center justify-center gap-2 pr-4">
            ...
            <div class="magnify cursor-pointer flex items-center justify-center gap-2">
              <Icon icon="mdi-light:magnify" class="text-3xl" />
              <p class="hidden lg:block text-lg">Search</p>
            </div>
          </div>
          <RouterLink to="/home" class="cursor-pointer">
            <div class="logo xl:ml-16 " ref="myLogo">
              <svg width="150" height="24" viewBox="0 0 210 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
          ...
          <div class="right-side-header flex gap-4">



            <div class="hidden cursor-pointer lg:flex wishlist-side  items-center gap-1">
              <Icon icon="ion:heart-outline" class="text-3xl" />
              <span class="text-lg px-2 hidden lg:block">Wishlist</span>
            </div>



            <div class="hidden cursor-pointer lg:flex cart-side  items-center gap-1">
              <Icon icon="clarity:user-line" class="text-3xl " />
              <span class="text-lg px-2 hidden lg:block">Sign in</span>
            </div>

            <div class="cart-side cursor-pointer flex items-center gap-1">
              <Icon icon="ion:cart-outline" class="text-3xl " />
              <span class="text-lg px-2 hidden lg:block">Cart</span>
              <span class="bg-gray-300 rounded-full px-3 py-1 text-lg select-none ">0</span>
            </div>
...
const openMobileNav = () => {
  mobileNav_activator.value = !mobileNav_activator.value;
  myMobileMenu.value.$el.classList.remove('-translate-x-full')
}
